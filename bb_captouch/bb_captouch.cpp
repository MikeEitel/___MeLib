...
//    Wire1.begin(iSDA, iSCL); // this is specific to ESP32 MCUs    CHANGED by ME
    Wire1.setClock(u32Speed);
...
int BBCapTouch::I2CWrite(uint8_t u8Addr, uint8_t *pData, int iLen)
{
  int rc = 0;
    ...
    Wire1.beginTransmission(u8Addr);
    Wire1.write(pData, (uint8_t)iLen);
...
int BBCapTouch::I2CReadRegister16(uint8_t u8Addr, uint16_t u16Register, uint8_t *pData, int iLen)
{
  ...
  Wire1.beginTransmission(u8Addr);
  Wire1.write((uint8_t)(u16Register>>8)); // high byte
  Wire1.write((uint8_t)u16Register); // low byte
